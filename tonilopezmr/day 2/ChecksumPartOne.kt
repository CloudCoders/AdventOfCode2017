...
class ChecksumPartOne {
  ...
  @Test
  fun `calculate checksum 8 + 4 + 6 = 18`() {
    val input = arrayOf(
        arrayOf(5, 1, 9, 5),
        arrayOf(7, 5, 3),
        arrayOf(2, 4, 6, 8)
    ...
    assertEquals(18, checksum(of = input))
  }

  @Test
  fun `calculate checksum for day 2`() {
    ...
    assertEquals(43074, checksum(day2Parser(input)))
  }

  @Test
  fun `given an text input with a matrix should return a matrix of ints`() {
    val matrixIntput = "1\t2\t3\n4\t5"
    val expectedMatrix = arrayOf(arrayOf(1,2,3), arrayOf(4, 5))

    assertArrayEquals(expectedMatrix, day2Parser(matrixIntput))
  }

}

fun day2Parser(input: String): Array<Array<Int>> = input.split("\n").map {
  it.split("\t").map { it.toInt() }.toTypedArray()
}.toTypedArray()